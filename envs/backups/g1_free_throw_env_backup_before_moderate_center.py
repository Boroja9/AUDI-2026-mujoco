...
W_ACCURACY = 25.0              # povecano (bilo 15.0) - vise poena sto si blizi centru
K_ACC = 2.5                    # povecano (bilo 1.5) - ostriji pad, jasnije razlikuje
                                # tacan centar od "samo unutar radijusa"
BULLSEYE_BONUS = 30.0          # povecano (bilo 20.0)
MAX_BALL_X = 12.0
W_RELEASE_VX = 1.0
...
class G1FreeThrowEnv(G1FixedBodyThrowEnv):
    ...
    def step(self, rl_action):
        ...
        landed = terminated and not self.robot_fell and info.get("landing_error") is not None
        ...
        if landed:
            ...
            if info.get("success"):
                r += BULLSEYE_BONUS
            if abs(ball_x) > MAX_BALL_X or abs(float(self._ball_pos()[1])) > MAX_BALL_X:
                terminated = True
        info["ball_x"] = ball_x
