...
W_RECOVER_CLOSE = 3.0      # kontinualna nagrada srazmerna blizini neutrali (0,0,0) - ne samo
                           # napredak, vec i sama blizina, svaki korak u recovery fazi
RECOVER_CLOSE_SCALE = 3.0  # rad - na ovoj udaljenosti bonus je ~0, na 0 rad bonus je pun W_RECOVER_CLOSE
W_RECOVER_HOLD = 5.0       # dodatni bonus SVAKI korak dok miruje blizu neutrale (vec "stigao")
W_RECOVER_SMOOTH = 0.02    # kazna za naglu promenu brzine zgloba (trzaj) tokom CELOG
                           # povratka, ne samo posle sto stigne - da se ne cima usput.
                           # (izmereno: jerk^2 prosek ~20, vrh ~156 na prelazu WHIP->
                           # recovery - W_RECOVER_SMOOTH=2.0 je eksplodiralo reward na
                           # -1130; ublazeno + kapirano ispod da jedan skok ne dominira)
RECOVER_JERK_CAP = 30.0
W_RECOVER_SPEED = 10.0        # JEDNOKRATAN bonus za brzinu stizanja do neutrale -
                              # sto brze stigne (manje vremena od WHIP_END), veci bonus
RECOVER_SPEED_WINDOW = 0.5    # s - stigne li unutar ovoga, pun bonus; posle linearno opada do 0
...
class G1FreeThrowEnv(G1FixedBodyThrowEnv):
    ...
    def reset(self, seed=None, options=None):
        ...
        self._recover_high_water = None  # ratchet: |vrednost| sme samo da opada ka 0 (neutrali)
        self._prev_recover_vel = None
        self._recover_entry_step = None
        self._recover_arrived_step = None
    ...
    def step(self, rl_action):
        ...
        if t < WHIP_END:
            ...
            # RATCHET (strukturalno, ne samo reward): |vrednost| (rastojanje
            # od neutrale=0 u ovom normalizovanom prostoru) sme SAMO da opada
            # iz koraka u korak, nikad da raste - sprecava da ruka nakratko
            # ode gore/u stranu (npr. ka glavi) pre nego sto se spusti, bez
            # obzira sta jos-netrenirana mreza izlaze.
            desired = np.clip(
                full[self.active_joints]
                + RECOVER_RESIDUAL_SCALE * arm_residual * self._recover_mask,
                -1, 1,
            )
            if self._recover_high_water is None:
                self._recover_high_water = desired.copy()
            else:
                closer = np.abs(desired) < np.abs(self._recover_high_water)
                self._recover_high_water = np.where(closer, desired, self._recover_high_water)
            full[self.active_joints] = self._recover_high_water
        ...
        if t >= WHIP_END:
            if self._recover_entry_step is None:
                self._recover_entry_step = self.step_count
            ...
            # kontinualna nagrada srazmerna samoj blizini neutrali (0,0,0) -
            # ne samo napredak vec i "koliko si blizu" svaki korak
            r += W_RECOVER_CLOSE * max(0.0, 1.0 - dist / RECOVER_CLOSE_SCALE)
            recover_vel = self.data.qvel[self._recover_qvel_adr]
            # kazna za trzaj (nagla promena brzine) tokom CELOG povratka, ne
            # samo posle sto stigne - da pokret bude gladak usput, ne da se cima
            if self._prev_recover_vel is not None:
                jerk = recover_vel - self._prev_recover_vel
                jerk_sq = min(float(np.dot(jerk, jerk)), RECOVER_JERK_CAP)
                r -= W_RECOVER_SMOOTH * jerk_sq
            self._prev_recover_vel = recover_vel.copy()
            if dist < RECOVER_SETTLE_EPS:
                r -= W_RECOVER_JITTER * float(np.linalg.norm(recover_vel))
                # dodatni bonus svaki korak dok miruje blizu neutrale (vec stigao)
                r += W_RECOVER_HOLD
                # JEDNOKRATAN bonus za brzinu stizanja - sto pre stigne (manje
                # vremena od WHIP_END), veci bonus
                if self._recover_arrived_step is None:
                    self._recover_arrived_step = self.step_count
                    elapsed = (self._recover_arrived_step - self._recover_entry_step) * self.control_dt
                    r += W_RECOVER_SPEED * max(0.0, 1.0 - elapsed / RECOVER_SPEED_WINDOW)
        else:
            self._prev_recover_dist = None
            self._prev_recover_vel = None
