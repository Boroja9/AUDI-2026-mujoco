...
class G1FreeThrowEnv(G1FixedBodyThrowEnv):
    ...
    def _baseline_action(self, t):
        ...
        if t < LOWER_END:
            # posle bacanja: ruka se spusta iz poze bacanja u neutralnu
            # (nominalnu) pozu - jednom, kontrolisano, pa vise ne mrda
            p = _smoothstep((t - WHIP_END) / (LOWER_END - WHIP_END))
            return self._A_throw + (self._A_neutral - self._A_throw) * p
        # ostaje u neutralnoj pozi do kraja epizode, bez daljeg pomeranja
