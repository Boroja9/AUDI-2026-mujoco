...
# ravno na podu (izmereno), sve iznad toga se nagradjuje.
FOOT_REST_Z = 0.033
W_FOOT_LIFT = 8.0
...
class G1WalkEnv(gym.Env):
    ...
    def step(self, action):
        ...
        progress = (capped_fwd - self._prev_capped_fwd) / self.control_dt
        ...
        left_lift = max(0.0, self.data.xpos[self.left_foot_id, 2] - FOOT_REST_Z)
        right_lift = max(0.0, self.data.xpos[self.right_foot_id, 2] - FOOT_REST_Z)
        r += W_FOOT_LIFT * (left_lift + right_lift)
